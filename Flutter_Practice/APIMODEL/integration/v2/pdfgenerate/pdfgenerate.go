...
func PDFGenerate(pDebug *helpers.HelperStruct, pTemplateData TemplateStruct, pUid, pSid string) (string, error) {
	...
	//get URL from toml
	lPDFGenerateUrl := (tomlconfig.GtomlConfigLoader.GetValueString("fileconfig", "baseUrl") + tomlconfig.GtomlConfigLoader.GetValueString("fileconfig", "PDFGenerateUrl"))
...
type MergeFileToPdfStruct struct {
	InputDocId  []AttachStruct `json:"input_docid"`
	ProcessType string         `json:"processtype"`
}
...
/*
Usage Of MergeFiletoPdf
 MergeFiletoPdf Method is Used to Merge both images and pdf in single pdf in final Result
 ...
 Request Structure
  ...
  Parameter :
   - pDebug *helper>He;perStruct
   - pProcssType ,pUid,pSid String

*/
func MergeFiletoPdf(pDebug *helpers.HelperStruct, pProcessType, pUid, pSid string, pDocID []AttachStruct) (PasswordFileStruct, error) {
	pDebug.Log(helpers.Statement, "MergeFiletoPdf (+)")
	var lFileReadRec PasswordFileStruct
	var lHeaderArr []apiUtil.HeaderDetails
	var lHeaderRec apiUtil.HeaderDetails

	var lMergeFileRec MergeFileToPdfStruct
	//read the value from toml file
	//get URL from toml
	lReadFileUrl := (tomlconfig.GtomlConfigLoader.GetValueString("fileconfig", "baseUrl") + tomlconfig.GtomlConfigLoader.GetValueString("fileconfig", "MergeFileToPDF"))
	//re-build the url adding QueryParameter
	...
	//call the api to given URL
	lResp, lErr := apiUtil.Api_call(pDebug, lReadFileUrl, "POST", string(lBodyData), lHeaderArr, "Merge PDF")
	if lErr != nil {
		return lFileReadRec, helpers.ErrReturn(lErr)
	}
...
func FillPDFFile(pDebug *helpers.HelperStruct, pTemplateData PDFFormStruct, pUid, pSid, pKey string) (string, error) {
