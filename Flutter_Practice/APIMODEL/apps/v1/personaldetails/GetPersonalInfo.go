...
func InsertFatcaDetails(Uid string, SessionId string, lPersonalInfo PersonalStruct, pDebug *helpers.HelperStruct) error {
	...
	(Request_uid,Residence_Country, Tax_Idendification_Number,Place_of_Birth, Country_of_Birth, Foreign_Address1, Foreign_Address2, Foreign_Address3, Foreign_City, Foreign_Country, Foreign_State, Foreign_Pincode, Session_Id, Updated_Session_Id, CreatedDate, UpdatedDate)
	VALUES(?,?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, unix_timestamp(now()),unix_timestamp(now()))`
...
func UpdateFatcaDetails(Uid string, SessionId string, lPersonalInfo PersonalStruct, pDebug *helpers.HelperStruct) error {
	...
	SET Residence_Country=?, Tax_Idendification_Number=?, Place_of_Birth=?, Country_of_Birth=?, Foreign_Address1=?, Foreign_Address2=?, Foreign_Address3=?, Foreign_City=?, Foreign_Country=?, Foreign_State=?, Foreign_Pincode=?, Session_Id=?, Updated_Session_Id=?, CreatedDate=unix_timestamp(now()), UpdatedDate=unix_timestamp(now())
	WHERE Request_uid=?`
