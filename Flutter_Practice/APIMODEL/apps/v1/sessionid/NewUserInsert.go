...
func NewUserdataInsert(pDebug *helpers.HelperStruct, pUserRec UserStruct, pReq *http.Request, pUid, pSid string) error {
	...
	pDebug.Log(helpers.Details, " pReq.Header.Get(App_mode)", pReq.Header.Get("App_mode"))
	// var lDeviceName string
	// if strings.EqualFold(pReq.Header.Get("App_mode"), "app") {
	// 	lDeviceName = "app"
	// } else {
	// 	lDeviceName = "web"
	// }
	...
	then
	insert into ekyc_request (uid,Given_Name,Phone,Email,Given_State,Created_Session_Id,Updated_Session_Id,CreatedDate,UpdatedDate,Form_Status,Name_As_Per_Pan,app)
	values(?,?,?,?,?,?,?,unix_timestamp(),unix_timestamp(),'OB',?,?);
	end if;`

	_, lErr := ftdb.NewEkyc_GDB.Exec(insertString, pUid, pUid, pUserRec.Name, pUserRec.Phone, pUserRec.Email, pUserRec.State, pSid, pSid, pUserRec.Name, lDeviceName)
	if lErr != nil {
...
func UpdateUserdata(pDebug *helpers.HelperStruct, pUid string, pUserRec UserStruct) error {
	...
	insertString := `  update ekyc_request 
						set Given_Name = ? , Name_As_Per_Pan = ?` + lTestUserUpd + `
						where Uid = ? `

	_, lErr := ftdb.NewEkyc_GDB.Exec(insertString, pUserRec.Name, pUserRec.Name, pUid)
	if lErr != nil {
		return helpers.ErrReturn(lErr)
	}
