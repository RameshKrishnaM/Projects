...
func ValidatePanReq(pDebug *helpers.HelperStruct, pPANData PanDataInfo, pTestUserRec TestuserStruct) (string, string) {
	...
	if strings.ToUpper(common.BOCheck) != "N" && !pTestUserRec.isTestUser {
		...
		lErr = PANNoCheck(pPANData.PanNumber, pDebug)
		if lErr != nil {
			pDebug.Log(helpers.Elog, lErr.Error())
			return "AA", "The given PAN number is already registered with us"
		}
	}
