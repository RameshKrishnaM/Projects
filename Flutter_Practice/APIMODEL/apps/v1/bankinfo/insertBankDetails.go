		...
		if lErr != nil {
			...
			if PDtotalCount >= lPDfixedCount {
				...
				if lErr != nil {
					...
					var lBankDetails pennydrop.BankDetails
					var lPennyDropResult pennydrop.ValidationResp
					var lErr error
					...
					if lErr != nil {
						...
						if lFlag == "N" {
							...
							lPennyDropResult, lErr = pennydrop.PennyDropValidation(lBankDetails)
							if lErr != nil {
								pDebug.Log(helpers.Elog, "IBD09"+lErr.Error())
								return "", helpers.ErrReturn(lErr)
							} else {
								if lPennyDropResult.PennyDropStatus == "completed" || lPennyDropResult.PennyDropStatus == "created" {
									pDebug.Log(helpers.Details, "Completed")
									lPennyDropStatus = "S"
									ActiveStatus := "Y"
									lErr := EkycBankInsert(lRequestId, lPennyDetails.Id, pBankInfo, ActiveStatus, lPennyDropResult, lPennyDropStatus, lSessionId, pDebug)
									if lErr != nil {
...
func EkycBankInsert(pRequestId string, pPdRefId int, pBankInfo BankStruct, pActiveStatus string, pPennyDropResult pennydrop.ValidationResp, pPennyDropStatus string, pSessionId string, pDebug *helpers.HelperStruct) error {
	...
	lCoreString := `insert into ekyc_bank (Request_Uid,Acc_Number,Acctype,IFSC,MICR,Bank_Name,Bank_Branch,Bank_Address,U_BankAddress,
		...
		`
	_, lErr = ftdb.NewEkyc_GDB.Exec(lCoreString, pRequestId, pBankInfo.ACCNO, pBankInfo.Acctype, pBankInfo.IFSC, pBankInfo.MICR, pBankInfo.BANK, pBankInfo.BRANCH, lAddress, lAddress, pPennyDropResult.PennyDropStatus, pPennyDropResult.AccountStatus, pActiveStatus, pPennyDropResult.RegisterName, pSessionId, pSessionId, pPdRefId)
	if lErr != nil {
