...
func GenerateIPVlink(w http.ResponseWriter, r *http.Request) {
	...
	if strings.EqualFold(r.Method, "GET") {
		...
		lIPVLinkExpTime := tomlconfig.GtomlConfigLoader.GetValueString("ipv", "IPVLinkExpTime")
		lIPVUrlRec, lErr := InsertIPVLinkInfo(lDebug, lUid, lSid, lIPVLinkExpTime)
		if lErr != nil {
		...
		lIPVUrlRec.Url = fmt.Sprintf("%s/ipvlink?uid=%s", lURL, lIPVUrlRec.IPVLinkID)
...
func IPVLinkStatusUpdate(pDebug *helpers.HelperStruct, pUid, pSid string) (lErr error) {
	pDebug.Log(helpers.Statement, "IPVLinkStatusUpdate (+)")

	pDebug.Log(helpers.Details, "pSid", pSid, "  ", "pUid", pUid)
	lUpdateQry := `update ekyc_ipv_link 
set isActive ='N',Updated_Session_Id=?,Updatedtime=unix_timestamp() 
where Request_Uid =?`

	_, lErr = ftdb.NewEkyc_GDB.Exec(lUpdateQry, pSid, pUid)
	if lErr != nil {
...
func InsertIPVLinkInfo(pDebug *helpers.HelperStruct, pUid, pSid, pIPVLinkExpTime string) (lIPVUrlRec IPVUrlStruct, lErr error) {
	...
	lErr = IPVLinkStatusUpdate(pDebug, pUid, pSid)
	if lErr != nil {
		return lIPVUrlRec, helpers.ErrReturn(lErr)
	}

	// lIntVal,strconv.Atoi(pIPVLinkExpTime)

	if strings.EqualFold(pIPVLinkExpTime, "") || !strings.Contains(pIPVLinkExpTime, ":") {
		return lIPVUrlRec, helpers.ErrReturn(errors.New("IPV Exp Time Will empty or not valid formet"))
	}
	pIPVLinkExpTime = strings.ReplaceAll(pIPVLinkExpTime, ":", " ")

	lIPVUrlRec.IPVLinkID = strings.ReplaceAll(uuid.NewV4().String(), "-", "")

	lCorestring := `INSERT INTO ekyc_ipv_link
	(Request_Uid, Session_Id, Updated_Session_Id,complit_Status, Createdtime, Updatedtime, Expiretime,use_status,ipv_session,isActive)
	VALUES(?,?,?,?,unix_timestamp(),unix_timestamp(),unix_timestamp(ADDDATE(now(), INTERVAL ? DAY_MINUTE)),?,?,'Y');`
	_, lErr = ftdb.NewEkyc_GDB.Exec(lCorestring, pUid, pSid, pSid, "N", pIPVLinkExpTime, "N", lIPVUrlRec.IPVLinkID)
	if lErr != nil {
		return lIPVUrlRec, helpers.ErrReturn(lErr)
	}
...
func UpdateIPVComplit(pDebug *helpers.HelperStruct, pSid, pUid string) (lErr error) {
	...
	lCorestring := `UPDATE ekyc_ipv_link
	SET complit_Status='Y',Updatedtime=unix_timestamp(),isActive='N'
...
func IPVLinkCheck(pDebug *helpers.HelperStruct, pActionType, pId string) (lCustemErr, lErr error) {
	...
	if strings.EqualFold(pActionType, "insert") {
		...
		lSelectQry = fmt.Sprintf("WHEN EXISTS(select * from ekyc_ipv_link eil where unix_timestamp(NOW()) between eil.Createdtime  and eil.Expiretime and eil.complit_Status='N' and eil.ipv_session ='%s'and eil.isActive ='Y')", pId)
