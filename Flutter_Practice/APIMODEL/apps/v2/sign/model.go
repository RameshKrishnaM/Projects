...
func Api_call_processed_data(processType string, inputJson string, pDebug *helpers.HelperStruct) (string, error) {
	...
	//ClosurePDFAPI := fmt.Sprintf("%v", kycUtilconfig.(map[string]interface{})["ClosurePDFAPI"])
	//BankAdditionPDFAPI := fmt.Sprintf("%v", kycUtilconfig.(map[string]interface{})["BankAdditionPDFAPI"])
	NomineePDFAPI := tomlconfig.GtomlConfigLoader.GetValueString("kycutilconfig", "NomineePDFAPI")
	//EsignRequestAPI := fmt.Sprintf("%v", kycUtilconfig.(map[string]interface{})["EsignRequestAPI"])
	//EsignDocumentAPI := fmt.Sprintf("%v", kycUtilconfig.(map[string]interface{})["EsignDocumentAPI"])
