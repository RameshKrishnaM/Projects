...
func GendratePDF(w http.ResponseWriter, r *http.Request) {
	...
	if strings.EqualFold("POST", r.Method) {
		...
		if strings.EqualFold(lDocID, "") {
			...
			lBoId, lIsNewID, lErr := GetBoid(lDebug, lUid)
			if lErr != nil {
			...
			// Log only if there is new Boid is Creadted.
			if lIsNewID {
				creditsmanage.LogVendorCredit(lDebug, "CDSLBOIDVndr", "CDSLBOIDSrv", lUid)
			}
			...
			lErr = commonpackage.UpdateDocID(lDebug, "SignedDocId", lRespRec.DocID, lUid, lSid)
			if lErr != nil {
			...
			lErr = commonpackage.AttachmentlogFile(lUid, "unSigned PDF", lRespRec.DocID, lDebug)
			if lErr != nil {
				lDebug.Log(helpers.Elog, "PGP05"+lErr.Error())
				fmt.Fprint(w, helpers.GetError_String("PGP05", "Somthing is wrong please try again later"))
				return
...
func CreatePDF(pDebug *helpers.HelperStruct, pUid, pSid, pNewClientcode, pBoID string) (string, error) {
	...
	lUserDataStruct.Date = time.Now().Format("02/01/2006")
	...
	// lUserDataStruct.StockBroker = tomlconfig.GtomlConfigLoader.GetValueString("dpscheme","StockBroker")
	...
	// lUserDataStruct.TradeOtherData = tomlconfig.GtomlConfigLoader.GetValueString("dpscheme","TradeOtherData")
	...
	var lPanProofRequiredKey, lPanProofDocId, lPanProofRequiredStatus string
	...
	pDebug.Log(helpers.Details, "lPanProofRequiredStatus", lPanProofRequiredStatus)

	lPanProofDocId = stages.SignedDocRec.PanImage

	if lPanProofRequiredStatus == "N" {

		lPanProofDocId = ""

		BlankPANProofRequired := tomlconfig.GtomlConfigLoader.GetValueString("fileconfig", "BlankPANProofRequired")

		pDebug.Log(helpers.Details, "BlankPANProofRequired", BlankPANProofRequired)

		lIsBlankPanProof := coresettings.GetCoreSettingValue(ftdb.NewEkyc_GDB, BlankPANProofRequired)
		pDebug.Log(helpers.Details, "lIsBlankPanProof", lIsBlankPanProof)

		if strings.EqualFold(lIsBlankPanProof, "Y") {
			BlankProofDocId := tomlconfig.GtomlConfigLoader.GetValueString("fileconfig", "BlankProofDocId")

			lPanProofDocId = coresettings.GetCoreSettingValue(ftdb.NewEkyc_GDB, BlankProofDocId)
			pDebug.Log(helpers.Details, "lBlankProofDocId", lPanProofDocId)

		}
	}
	lAttachArr1 := Attachment(pDebug, lPanProofDocId, lAddressProofdocId, stages.SignedDocRec.SignImage)
...
func EKYCPDFGenerate(pDebug *helpers.HelperStruct, pJsonData, pUid, pSid string, pImageInfo []pdfgenerate.ImgStruct, pAttacMent1, pAttacMent2 []pdfgenerate.AttachStruct, stages stage) (string, error) {
	...
	lDocID1, lErr := pdfgenerate.FillPDFFile(pDebug, lPDFRormRec, pUid, pSid, "key")
	if lErr != nil {
		return "", helpers.ErrReturn(lErr)
	}
...
func CreateClientId(pReqid string, pDebug *helpers.HelperStruct) (string, error) {
	...
	lClientId, lErr := CheckClientIdIsExistsOrNot(pReqid, pDebug)
	if lErr != nil {
		return "", helpers.ErrReturn(lErr)
	}

	if lClientId == "" {
		lClientId, lApplicationNo, lErr := GenerateClientId()
		if lErr != nil {
			return "", helpers.ErrReturn(lErr)
		}

		lErr = UpdateClientIdAndAppNo(lClientId, lApplicationNo, pReqid)
		if lErr != nil {
			return "", helpers.ErrReturn(lErr)
		}

		pDebug.Log(helpers.Details, "CLIENT ID =>", lClientId, " APPLICATION NO => ", lApplicationNo)
	}
	pDebug.Log(helpers.Statement, "CreateClientId (-)")
	return lClientId, nil
}

func CheckClientIdIsExistsOrNot(pReqid string, pDebug *helpers.HelperStruct) (string, error) {

	// Log the start of the CheckClientIdIsExistsOrNot function
	pDebug.Log(helpers.Statement, "CheckClientIdIsExistsOrNot(+)")

	// Varibale declaration
	var lClientId string

	lCoreString := `SELECT nvl(er.Client_Id,"") FROM ekyc_request er WHERE er.Uid = ?`
	...
	defer lRows.Close()
	for lRows.Next() {
		lErr := lRows.Scan(&lClientId)
		pDebug.Log(helpers.Details, "lRows", lRows)
		if lErr != nil {
			return "", helpers.ErrReturn(lErr)
		}
	}
	// Log the end of the CheckClientIdIsExistsOrNot function
	pDebug.Log(helpers.Statement, "CheckClientIdIsExistsOrNot(-)")

	return lClientId, nil
}

func UpdateClientIdAndAppNo(pClientId, pApplicationNo, pReqid string) error {
	// Log the start of the UpdateRecords function
	log.Println("UpdateClientIdAndAppNo (+)")
	...
	lSqlString := `update ekyc_request set Client_Id = ? ,applicationNo=?
					  where Uid=?`

	// Execute the SQL update query with the provided parameter
	lExecResult, lErr := ftdb.NewEkyc_GDB.Exec(lSqlString, pClientId, pApplicationNo, pReqid)
	if lErr != nil {
		...
		return fmt.Errorf("UpdateClientIdAndAppNo - (AUR-002) " + lErr.Error())
	}
	...
	log.Printf("UpdateClientIdAndAppNo Rows affected: %d\n", lRowsAffected)
	log.Println("Record Updated successfully")

	// Log the end of the UpdateClientIdAndAppNo function
	log.Println("UpdateClientIdAndAppNo (-)")
	return nil
}
...
func CheckChangesInForm(pDebug *helpers.HelperStruct, pUid string) (lDocID string, lErr error) {
	...
	lQry := `select NVL(er.unsignedDocid,'')
