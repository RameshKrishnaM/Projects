...
func GetBoid(pDebug *helpers.HelperStruct, pReqId string) (string, bool, error) {
	pDebug.Log(helpers.Statement, "GetBoid (+)")
	var lBoid string
	var lisBOidCreated bool
	...
	if lErr != nil {
		...
		return "", lisBOidCreated, helpers.ErrReturn(lErr)
	}
	defer lRows.Close()
	for lRows.Next() {
	...
	if lBoid == "0" {
		...
		lisBOidCreated = false
	}

	pDebug.Log(helpers.Statement, "GetBoid (-)")
	return lBoid, lisBOidCreated, nil
}
