...
func AddressInfo(pRequestId string, ReturnStruct stage, pDebug *helpers.HelperStruct, pLookUpRec commonpackage.DescriptionResp) (stage, error) {
	...
	lCorestring := `select nvl(Source_Of_Address,""),nvl(CorAddress1,""),nvl(CorAddress2,""),nvl(CorAddress3,""),nvl(CorCity,""),nvl(CorState,""),nvl(CorPincode,""),nvl(CorCountry,""),nvl(PerAddress1,""),nvl(PerAddress2,""),nvl(PerAddress3,""),nvl(PerCity,""),nvl(PerState,""),nvl(PerPincode,""),nvl(PerCountry,""),nvl(proofType,""),nvl(dateofProofIssue,""),nvl(Proof_No,""),nvl(ProofOfIssue,""),nvl(ProofExpriyDate,""),nvl(Proof_Doc_Id1,""),nvl(Proof_Doc_Id2,""),nvl(Kra_docid,""),nvl(Digilocker_docid,""),nvl(Cor_Address_DocId1,""),nvl(Cor_Address_DocId2,""),nvl(COR_ProofType,""),nvl(COR_ProofOfIssue,""),nvl(COR_ProofNo,""),nvl(COR_ProofExpiryDate,""),nvl(COR_ProofDateIssue,"") from ekyc_address where Request_Uid = ?`
	lRows, lErr := ftdb.NewEkyc_GDB.Query(lCorestring, pRequestId)
	if lErr != nil {
		return ReturnStruct, helpers.ErrReturn(lErr)
	}
	...
	for lRows.Next() {
		lErr := lRows.Scan(&ReturnStruct.AdrsRec.Source_Of_Address,
			...
			&ReturnStruct.AdrsRec.CORDocId1,
			&ReturnStruct.AdrsRec.CORDocId2,
			&ReturnStruct.AdrsRec.CORAdrsProofName,
			&ReturnStruct.AdrsRec.CORAdrsProofPlaceIsu,
			&ReturnStruct.AdrsRec.CORAdrsProofNo,
			&ReturnStruct.AdrsRec.CORProofExpriyDate,
			&ReturnStruct.AdrsRec.CORAdrsproofIsuDate)
		...
		if ReturnStruct.AdrsRec.CORAdrsProofName != "" {
			pLookUpRec, lErr = commonpackage.GetLookUpDescription(pDebug, "AddressProof", ReturnStruct.AdrsRec.CORAdrsProofName, "code")
			if lErr != nil {
				return ReturnStruct, helpers.ErrReturn(lErr)
			}
			ReturnStruct.AdrsRec.CORAdrsProofName = pLookUpRec.Descirption
		}
...
func FetchFatcaDetails(Uid string, ReturnStruct stage, pDebug *helpers.HelperStruct) (stage, error) {
	...
	if lerr != nil {
		...
		defer rows.Close()
		for rows.Next() {
			lErr := rows.Scan(&ReturnStruct.PersonalRec.ResidenceCountry, &ReturnStruct.PersonalRec.TaxIdendificationNumber, &ReturnStruct.PersonalRec.PlaceofBirth, &ReturnStruct.PersonalRec.CountryofBirth, &ReturnStruct.PersonalRec.ForeignAddress1, &ReturnStruct.PersonalRec.ForeignAddress2, &ReturnStruct.PersonalRec.ForeignAddress3, &ReturnStruct.PersonalRec.ForeignCity, &ReturnStruct.PersonalRec.ForeignCountry, &ReturnStruct.PersonalRec.ForeignState, &ReturnStruct.PersonalRec.ForeignPincode, &ReturnStruct.PersonalRec.FatcaTaxExempt, &ReturnStruct.PersonalRec.FatcaTaxExemptReason)
			if lErr != nil {
				return ReturnStruct, helpers.ErrReturn(lerr)
			}
		}
	}
	if ReturnStruct.PersonalRec.FatcaTaxExemptReason != "" {
		lDescription, lErr := commonpackage.GetLookUpDescription(pDebug, "FatcaExemptReason", ReturnStruct.PersonalRec.FatcaTaxExemptReason, "code")
		if lErr != nil {
			pDebug.Log(helpers.Elog, "AI0016"+lErr.Error())
			return ReturnStruct, lErr
		}
		ReturnStruct.PersonalRec.FatcaTaxExemptReason = lDescription.Descirption
	}
	if ReturnStruct.PersonalRec.ResidenceCountry != "" {
		lDescription, lErr := commonpackage.GetLookUpDescription(pDebug, "FatcaCountry", ReturnStruct.PersonalRec.ResidenceCountry, "code")
		if lErr != nil {
			pDebug.Log(helpers.Elog, "AI0016"+lErr.Error())
			return ReturnStruct, lErr
		}
		ReturnStruct.PersonalRec.ResidenceCountry = lDescription.Descirption
	}
	if ReturnStruct.PersonalRec.CountryofBirth != "" {
		lDescription, lErr := commonpackage.GetLookUpDescription(pDebug, "FatcaCountry", ReturnStruct.PersonalRec.CountryofBirth, "code")
		if lErr != nil {
			pDebug.Log(helpers.Elog, "AI0016"+lErr.Error())
			return ReturnStruct, lErr
		}
		ReturnStruct.PersonalRec.CountryofBirth = lDescription.Descirption
	}
	if ReturnStruct.PersonalRec.ForeignCountry != "" {
		lDescription, lErr := commonpackage.GetLookUpDescription(pDebug, "FatcaCountry", ReturnStruct.PersonalRec.ForeignCountry, "code")
		if lErr != nil {
			pDebug.Log(helpers.Elog, "AI0016"+lErr.Error())
			return ReturnStruct, lErr
		}
		ReturnStruct.PersonalRec.ForeignCountry = lDescription.Descirption
	}
	pDebug.Log(helpers.Statement, "FetchFatcaDetails (-)")
	return ReturnStruct, nil
}
...
func FileUpload(pRequestId string, ReturnStruct stage, pDebug *helpers.HelperStruct, pLookUpRec commonpackage.DescriptionResp) (stage, error) {
	...
	defer lRows.Close()
	// ReturnStruct.SignedDocRec.SignFlag = "N"
	for lRows.Next() {
	...
	ReturnStruct.SignedDocRec.SignFlag = "N"
	Wet_SignFlag := tomlconfig.GtomlConfigLoader.GetValueString("ipv", "Wet_Sign_Flag")

	ReturnStruct.IpvRec.SignatureId = ""
	if strings.EqualFold(Wet_SignFlag, "Y") {
		ReturnStruct.IpvRec.SignatureId = ReturnStruct.SignedDocRec.SignImage
	} else {
		ReturnStruct.SignedDocRec.SignFlag = "Y"
	}
