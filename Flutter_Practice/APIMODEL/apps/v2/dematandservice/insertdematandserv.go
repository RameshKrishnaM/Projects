...
func CashOnlyUpdate(pDebug *helpers.HelperStruct, pUid, pSid string) (lErr error) {
	pDebug.Log(helpers.Statement, "CashOnlyUpdate (+)")

	lQuery := `
		UPDATE ekyc_services
		SET selected = CASE
			WHEN segement_id IN (1) THEN 'Y'
			ELSE 'N'
		END,
		u_selected = CASE
			WHEN segement_id IN (1) THEN 'Y'
	...
	_, lErr = ftdb.NewEkyc_GDB.Exec(lQuery, pSid, pUid)
	if lErr != nil {
		return helpers.ErrReturn(lErr)
	}
	pDebug.Log(helpers.Statement, "CashOnlyUpdate (-)")
...
func IncomeProofFlag(pDebug *helpers.HelperStruct, pUid, pTestUserFlag string) (lErr error) {
	...
	if lIncomeFlag != 1 && !strings.EqualFold(pTestUserFlag, "0") {
