...
func GenerateSignReq(pDebug *helpers.HelperStruct, pEsignInfoRec ESignInfoStruct, pUid, pSid string) (lRespRec respStruct, lErr error) {
	...
	//Digio Credits Validity days readed from toml
	lEsignRec.ExpInDates, lErr = strconv.Atoi(tomlconfig.GtomlConfigLoader.GetValueString("digioesign", "DigioExpiryDate"))
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR000", helpers.ErrReturn(lErr))
	...
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR001", helpers.ErrReturn(lErr))
		return lRespRec, helpers.ErrReturn(lErr)
	}

	lDocIDMap, lErr := GetDocID(pDebug, pUid)
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR002", helpers.ErrReturn(lErr))
	...
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR003", helpers.ErrReturn(lErr))

		return lRespRec, helpers.ErrReturn(lErr)
	}
	var lPageCount pdfgenerate.PageCount

	//Fetch the PanProofRequired data from toml
	...
	var lPanProofDocId, lIsBlankPanProof string

	//Condition to be checked if the pan proof docid is present and pan proof is required
	if lPanProofRequiredStatus == "Y" && lDocIDMap["PanDocid"] != "" {
		lPanProofDocId = lDocIDMap["PanDocid"]
		...
		lBlankPANProofDocId := tomlconfig.GtomlConfigLoader.GetValueString("fileconfig", "BlankProofDocId")

		pDebug.Log(helpers.Details, "BlankPANProofRequired", BlankPANProofRequired)
		pDebug.Log(helpers.Details, "lBlankPANProofDocId", lBlankPANProofDocId)

		//This method is used to fetch the Blankwhite proof is required or not from the coresetting
		lIsBlankPanProof = coresettings.GetCoreSettingValue(ftdb.NewEkyc_GDB, BlankPANProofRequired)

		//This method is used to fetch the Blankwhite proof docid  from the coresetting
		lIsBlankPanProofDocId := coresettings.GetCoreSettingValue(ftdb.NewEkyc_GDB, lBlankPANProofDocId)
		pDebug.Log(helpers.Details, "lIsBlankPanProof", lIsBlankPanProof)

		if lIsBlankPanProof == "Y" {
			lPanProofDocId = lIsBlankPanProofDocId
		}
	...
	if lPanProofDocId != "" {
		...
		lPageCount, lErr = pdfgenerate.GetPageCount(pDebug, lPanProofDocId, lDocIDMap["SignDocid"], lAddressProofdocId)

		if lErr != nil {
			pDebug.Log(helpers.Details, "GSR004", helpers.ErrReturn(lErr))

			return lRespRec, helpers.ErrReturn(lErr)
		}
	} else {

		lPageCount, lErr = pdfgenerate.GetPageCount(pDebug, lDocIDMap["SignDocid"], lAddressProofdocId)

		if lErr != nil {
			pDebug.Log(helpers.Details, "GSR0004", helpers.ErrReturn(lErr))
	...
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR005", helpers.ErrReturn(lErr))
	...
	log.Printf("lCoordinateInfo2 %+v", lCoordinateInfo)

	if lIsBlankPanProof == "Y" {
		delete(lCoordinateInfo, "3")
	}
	log.Printf("lCoordinateInfo3 %+v", lCoordinateInfo)
	...
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR006", helpers.ErrReturn(lErr))
	...
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR007", helpers.ErrReturn(lErr))
	...
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR008", helpers.ErrReturn(lErr))
	...
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR009", helpers.ErrReturn(lErr))
	...
	if lErr != nil {
		pDebug.Log(helpers.Details, "GSR010", helpers.ErrReturn(lErr))
		return lRespRec, helpers.ErrReturn(lErr)
	}
...
func getDefaultPageCoordinates(pDebug *helpers.HelperStruct, DocId, pProcessType string) (int, StampPositionStruct, error) {
	...
	if lErr != nil {
		pDebug.Log(helpers.Elog, "GDPC01"+lErr.Error())
