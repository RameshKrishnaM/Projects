...
func InsertZohoCrmDeal(pReq *http.Request, pRespWriter http.ResponseWriter, lZohoInsRec *zohointegration.ZohoCrmDealInsertStruct, pDebug *helpers.HelperStruct, pUid, pSessionId string) error {
	...
	//
	lErr := GetTempClientDetails(pDebug, pUid, lZohoInsRec, &lCrmDealReq)
	if lErr != nil {
	...
	if strings.EqualFold(common.InformCRM, "yes") {
...
=====================================
*/
func UpdatePhoneZohoCrmDeals(pDebug *helpers.HelperStruct, pReq *http.Request, pUid, pSessionId string) error {
	pDebug.Log(helpers.Statement, "UpdatePhoneZohoCrmDeals (+)")
	var lCrmDealReq zohointegration.ZohoCrmDealStruct
	var lZohoInsRec zohointegration.ZohoCrmDealInsertStruct

	SetCrmStages()
	pStage := common.MobileVerified
	...
	_ = GetUtmCookie(pDebug, pReq, &lZohoInsRec, &lCrmDealReq)
...
func UpdatePhoneZohoCrmDeals2(pDebug *helpers.HelperStruct, pReq *http.Request, pOldUid, pNewUid, pSessionId string) error {
	pDebug.Log(helpers.Statement, "UpdatePhoneZohoCrmDeals (+)")
	var lCrmDealReq zohointegration.ZohoCrmDealStruct
	var lZohoInsRec zohointegration.ZohoCrmDealInsertStruct
	...
	pDebug.Log(helpers.Details, "pOldUid --- ", pOldUid)

	lErr := GetClientDetails(pDebug, pOldUid, &lZohoInsRec, &lCrmDealReq)
	if lErr != nil {
		pDebug.Log(helpers.Elog, "SIUZCD03 ", lErr.Error())
		return helpers.ErrReturn(lErr)
	}
	...
	lErr = NewExistingDeal(pDebug, pNewUid, &lZohoInsRec, &lCrmDealReq)
	if lErr != nil {
		pDebug.Log(helpers.Elog, "SIUZCD04 ", lErr.Error())
		return helpers.ErrReturn(lErr)
	}
	...
	lErr = InsertZohoCrmDeals(pDebug, pNewUid, pSessionId, lDeviceInfo, &lZohoInsRec)
	if lErr != nil {
		pDebug.Log(helpers.Elog, "SIUZCD05 ", lErr.Error())
		return helpers.ErrReturn(lErr)
	}
...
func UpdateEmailZohoCrmDeals(pDebug *helpers.HelperStruct, pReq *http.Request, pUid, pStage, pSessionId string) error {
	pDebug.Log(helpers.Statement, "UpdateEmailZohoCrmDeals (+)")
	var lCrmDealReq zohointegration.ZohoCrmDealStruct
	var lZohoInsRec zohointegration.ZohoCrmDealInsertStruct
	...
	pDebug.Log(helpers.Details, "pUid --- ", pUid)
...
func GetTempClientDetails(pDebug *helpers.HelperStruct, pReqUid string, pZohoRec *zohointegration.ZohoCrmDealInsertStruct, lCrmDealReq *zohointegration.ZohoCrmDealStruct) error {
	pDebug.Log(helpers.Statement, "GetTempClientDetails (+)")
	var lStateCode string
	lCoreString := `select nvl( Given_Name,'') Name ,nvl(Email , '') Email ,nvl(Phone  , '') Phone  ,nvl(Given_State  , '') State 
	from ekyc_prime_request  
	where Uid = ?`

	pDebug.Log(helpers.Details, "pUid =>", pReqUid)
	...
	defer lRows.Close()
	for lRows.Next() {
		lErr = lRows.Scan(&pZohoRec.Clientname, &pZohoRec.Email, &pZohoRec.Phone, &lStateCode)
		if lErr != nil {
	...
	pDebug.Log(helpers.Details, "lStateCode =>", lStateCode, "pZohoRec.Clientname =>", pZohoRec.Clientname, "pZohoRec.Email =>", pZohoRec.Email, "pZohoRec.Phone =>", pZohoRec.Phone)
...
func SetCrmStages() {
	...
	common.InformCRM = tomlconfig.GtomlConfigLoader.GetValueString("crmdealconfig", "InformCRM")
	common.MobileVerified = tomlconfig.GtomlConfigLoader.GetValueString("crmdealconfig", "STAGE_PHONE")
	common.EmailVerified = tomlconfig.GtomlConfigLoader.GetValueString("crmdealconfig", "STAGE_EMAIL")
