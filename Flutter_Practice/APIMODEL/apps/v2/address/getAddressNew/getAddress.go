...
type DbRespStruct struct {
	...
	CORAProofofAddresscode string `json:"coradrsproofcode"`
...
Date : 24-July-2024
*/
func GetAddressNew(w http.ResponseWriter, r *http.Request) {
	lDebug := new(helpers.HelperStruct)
	lDebug.SetUid(r)
	lDebug.Log(helpers.Statement, "GetAddressNew (+)")
	...
	if r.Method == "GET" {
		...
		lCorestring := `select nvl(id,""),nvl(Source_Of_Address,""),nvl(CorAddress1,""),nvl(CorAddress2,""),nvl(CorAddress3,""),nvl(CorCity,""),nvl(CorState,""),
nvl(CorPincode,""),nvl(CorCountry,""),nvl(PerAddress1,""),nvl(PerAddress2,""),nvl(PerAddress3,""),
nvl(PerCity,""),nvl(PerState,""),nvl(PerPincode,""),nvl(PerCountry,""),nvl(proofType,""),nvl(dateofProofIssue,""),nvl(Proof_No,""),
nvl(ProofOfIssue,""),nvl(ProofExpriyDate,""),nvl(Proof_Doc_Id1,""),nvl(Proof_Doc_Id2,""),nvl(Cor_Address_DocId1,""),nvl(Cor_Address_DocId2,""),
nvl(Kra_docid,""),nvl(Digilocker_docid,""),nvl(SameAsPermenentAddress,""),nvl(COR_ProofType,""),nvl(COR_ProofOfIssue,""),nvl(COR_ProofNo,""),nvl(COR_ProofExpiryDate,""),nvl(COR_ProofDateIssue,"")
			from ekyc_address 
			where Request_Uid = ?
		...
		if lErr != nil {
			...
			defer lRows.Close()
			for lRows.Next() {
				lErr := lRows.Scan(&id, &lResp.Source_Of_Address, &lResp.CORAddress1, &lResp.CORAddress2, &lResp.CORAddress3, &lResp.CORCity, &lResp.CORState, &lResp.CORPincode, &lResp.CORCountry, &lResp.PERAddress1, &lResp.PERAddress2, &lResp.PERAddress3, &lResp.PERCity, &lResp.PERState, &lResp.PERPincode, &lResp.PERCountry, &lResp.ProofofAddresscode, &lResp.PERDateofissue, &lResp.PERProofNo, &lResp.PERProofPlaceofissue, &lResp.PERProofExpriyDate, &lResp.PERDocID1, &lResp.PERDocID2, &lResp.CORDocId1, &lResp.CORDocId2, &lkraId, &lDigilockerId, &lSignal, &lResp.CORAProofofAddresscode, &lResp.CORAdrsProofPlaceIsu, &lResp.CORAdrsProofNo, &lResp.CORProofExpriyDate, &lResp.CORAdrsproofIsuDate)
				...
				if lResp.CORAProofofAddresscode != "" {
					lResponse, lErr = commonpackage.GetLookUpDescription(lDebug, "AddressProof", lResp.CORAProofofAddresscode, "Code")
					if lErr != nil {
						lDebug.Log(helpers.Elog, "GA10 "+lErr.Error())
						fmt.Fprint(w, helpers.GetError_String("GA10 ", lErr.Error()))
						return
					}
					lResp.CORAdrsProofName = lResponse.Descirption
				}
				if lResp.CORState != "" {
