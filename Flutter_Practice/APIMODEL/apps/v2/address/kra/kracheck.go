...
type DbAddressStatusStruct struct {
	...
	ManualOption      string `json:"manualoption"`
	AllowModification string `json:"allowmodification"`
...
func AddressStatus(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		lResp.ManualOption = tomlconfig.GtomlConfigLoader.GetValueString("kra", "AllowManualAddrEntry")
		if strings.EqualFold(lTestUserFlag, "0") {
			lResp.ManualOption = "Y"
		}

		lResp.AllowModification = tomlconfig.GtomlConfigLoader.GetValueString("kra", "AllowModification")
		if strings.EqualFold(lTestUserFlag, "0") {
			lResp.AllowModification = "Y"
		}
