			...
			if LoggedBy != "" {
				...
				if err != nil {
					...
					if isStaff == "Y" {
						...
						publicCookie, err = req.Cookie("ftwl_ct") // when using comman it cause error
						if err != nil {
							log.Println(err)
							log.Println("ValidateAndGetClientDetails2-")
							//return "", fmt.Errorf("Invalid Session (0x901)")
...
func CheckIsStaff(clientId string) (string, error) {
	...
	rows, err := ftdb.MariaEKYCPRD_GDB.Query(CoreString, clientId)
	if err != nil {
		log.Println(err)
		return IsStaff, err
	} else {
...
func CheckPageTokenValidity2(token string, app string) string {
	...
	rows, err := ftdb.MariaFTPRD_GDB.Query(sqlString, token, app)
	if err != nil {
...
func IsAuthTokenValid(app string, token string, client string) string {
	...
	if err != nil {
		...
		if tokenid > 0 {
			...
			_, err := ftdb.MariaFTPRD_GDB.Exec(updString, tokenid)
			if err != nil {
