...
func (h *HelperStruct) Log(pDebugLevel int, pMsg ...interface{}) {
	...
	Console := tomlconfig.GtomlConfigLoader.GetValueString("debug", "PrintConsole")
	PrintConsole, err := strconv.Atoi(Console)
	if err != nil {
	...
	if (pDebugLevel <= intlevel && intlevel != 0) || pDebugLevel == Elog {
		...
		if strings.EqualFold(h.Sid, "") {
			...
			if PrintConsole == 1 {
				fmt.Println("Set the id before <-- debug := new(helpers.HelperStruct) after debug.Init(reference value) -->")
			}
		} else {
			...
			if lReference == "" || strings.EqualFold(lReference, h.Reference) {
				...
				if strings.Contains(lFinal, " @@ ") && strings.Contains(lFinal, "@@ ln") {
					...
					if PrintConsole == 1 {
						fmt.Println("@@", h.Sid, "@@ (", h.Reference, ") @@", lFinal)
					}
				} else {
					log.Println("@@", h.Sid, "@@ (", h.Reference, ") @@", lFilename, "@@", lFuncname, "@@ ln", lLine, "@@", lFinal)
					if PrintConsole == 1 {
						fmt.Println("@@", h.Sid, "@@ (", h.Reference, ") @@", lFilename, "@@", lFuncname, "@@ ln", lLine, "@@", lFinal)
					}
				}
			}
		}
