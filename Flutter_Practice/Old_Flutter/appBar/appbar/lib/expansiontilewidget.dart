...
/// Enables control over a single [ExpansionTileWidget]'s expanded/collapsed state.
///
/// It can be useful to expand or collapse an [ExpansionTileWidget]
/// programmatically, for example to reconfigure an existing expansion
/// tile based on a system event. To do so, create an [ExpansionTileWidget]
...
/// the [ExpansionTileWidget] to rebuild, so they may not be called from
/// a build method.
class ExpansionTileController {
  /// Create a controller to be used with [ExpansionTileWidget.controller].
  ExpansionTileController();

  _ExpansionTileWidgetState? _state;

  /// Whether the [ExpansionTileWidget] built with this controller is in expanded state.
  ///
  ...
  /// See also:
  ///
  ///  * [expand], which expands the [ExpansionTileWidget].
  ///  * [collapse], which collapses the [ExpansionTileWidget].
  ///  * [ExpansionTileWidget.controller] to create an ExpansionTile with a controller.
  bool get isExpanded {
    assert(_state != null);
    return _state!._isExpanded;
  }

  /// Expands the [ExpansionTileWidget] that was built with this controller;
  ///
  ...
  ///
  /// Calling this method may cause the [ExpansionTileWidget] to rebuild, so it may
  /// not be called from a build method.
  ///
  /// Calling this method will trigger an [ExpansionTileWidget.onExpansionChanged] callback.
  ///
  /// See also:
  ///
  ...
  /// Collapses the [ExpansionTileWidget] that was built with this controller.
  ///
  ...
  /// Finds the [ExpansionTileController] for the closest [ExpansionTileWidget] instance
  /// that encloses the given context.
  ///
  /// If no [ExpansionTileWidget] encloses the given context, calling this
  /// method will cause an assert in debug mode, and throw an
  /// exception in release mode.
  ///
  /// To return null if there is no [ExpansionTileWidget] use [maybeOf] instead.
  ///
  /// {@tool dartpad}
  /// Typical usage of the [ExpansionTileController.of] function is to call it from within the
  /// `build` method of a descendant of an [ExpansionTileWidget].
  ///
  /// When the [ExpansionTileWidget] is actually created in the same `build`
  ...
  /// would have an outer widget that creates the [ExpansionTileWidget]
  ...
  static ExpansionTileController of(BuildContext context) {
    final _ExpansionTileWidgetState? result =
        context.findAncestorStateOfType<_ExpansionTileWidgetState>();
    if (result != null) {
  ...
  /// Finds the [ExpansionTileWidget] from the closest instance of this class that
  /// encloses the given context and returns its [ExpansionTileController].
  ///
  /// If no [ExpansionTileWidget] encloses the given context then return null.
  /// To throw an exception instead, use [of] instead of this function.
  ///
  /// See also:
  ///
  ///  * [of], a similar function to this one that throws if no [ExpansionTileWidget]
  ///    encloses the given context. Also includes some sample code in its
  ///    documentation.
  static ExpansionTileController? maybeOf(BuildContext context) {
    return context
        .findAncestorStateOfType<_ExpansionTileWidgetState>()
        ?._tileController;
  }
}
...
/// to the [leading] and [trailing] properties of [ExpansionTileWidget].
///
/// {@tool dartpad}
/// This example demonstrates how the [ExpansionTileWidget] icon's location and appearance
/// can be customized.
///
...
/// programmatically expand or collapse an [ExpansionTileWidget].
///
/// ** See code in examples/api/lib/material/expansion_tile/expansion_tile.1.dart **
/// {@end-tool}
///
...
class ExpansionTileWidget extends StatefulWidget {
  ...
  ///
  /// {@tool dartpad}
  /// This sample showcases how to override the [ExpansionTileWidget] expansion
